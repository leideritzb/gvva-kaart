...
(function() {
  ...
  function init() {
    ...
    style.textContent = [
      ...
      '.kaart-overlay .wp:hover path, .kaart-overlay .wp:hover polygon { fill:rgba(0,0,0,.22); }',
      '.kaart-overlay .wp.actief path, .kaart-overlay .wp.actief polygon { fill:rgba(0,0,0,.35); }',
    ...
    // Achtergrond afbeelding (volledige kaart)
    ...
    // Overlay SVG laden (inline — geen CORS)
    var xhr = new XMLHttpRequest();
    xhr.open('GET', SVG_URL, true);
    xhr.onload = function() {
      if (xhr.status !== 200) return;

      // Parser gebruiken voor veilige SVG injectie
      ...
      // Wijken klikbaar maken
      var groups = svgEl.querySelectorAll('#dorpen_en_wijken_kleuren > g');
      groups.forEach(function(g) {
        g.classList.add('wp');
        // Maak vlak transparant maar klikbaar
        g.querySelectorAll('path, polygon').forEach(function(el) {
      ...
      // Sync hoogte overlay met img
      function syncSize() {
        svgEl.style.height = img.offsetHeight + 'px';
      }
